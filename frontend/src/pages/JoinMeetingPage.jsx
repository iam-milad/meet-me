...
const JoinMeetingPage = () => {
  ...
  const handleJoin = () => {
    ...
    if (!personalCode.trim()) {
      setPersonalCodeError("Personal code is required");
      hasError = true;
    } else {
          ...
          <div className="flex flex-col gap-5">
            <div className="flex flex-col gap-1">
              <Input
                id="personal-code"
                type="text"
                placeholder="Enter callee's personal code"
                required
                value={personalCode}
                onChange={(e) => {
                  setPersonalCode(e.target.value);
                  if (personalCodeError) setPersonalCodeError("");
                }}
              />
              {personalCodeError && <p className="text-xs text-red-500 ml-2">{personalCodeError}</p>}
            </div>
            <div className="flex flex-col gap-1">
              <Input
                id="name"
